...
const run = (client, interaction, respond) => {
  ...
  if (channels.has(thread.id)) {
    try {
      ...
      respond('❌ Issue', 'Bot failed to remove channel from database. Sorry about that. Please open a ticket.', '#ff0000', true);
    }
  }
  else {
    try {
      ...
      console.error(err);
      respond('❌ Issue', 'Bot failed to add channel to watchlist. Sorry about that. Please open a ticket.', '#ff0000', true);
    }
  }
};
