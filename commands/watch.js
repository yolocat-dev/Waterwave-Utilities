...
const run = (client, interaction, respond) => {
  ...
  if (threads.has(thread.id)) {
    try {
      ...
      respond('❌ Issue', 'Bot failed to remove thread from database. Sorry about that. Please open a ticket.', '#ff0000', true);
    }
  }
  else {
    try {
      ...
      console.error(err);
      respond('❌ Issue', 'Bot failed to add thread to watchlist. Sorry about that. Please open a ticket.', '#ff0000', true);
    }
  }
};
